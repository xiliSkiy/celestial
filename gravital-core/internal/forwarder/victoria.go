...
// VictoriaMetricsForwarder VictoriaMetrics 转发器
// VictoriaMetrics 兼容 Prometheus Remote Write 协议
type VictoriaMetricsForwarder struct {
...
// NewVictoriaMetricsForwarder 创建 VictoriaMetrics 转发器
func NewVictoriaMetricsForwarder(config *ForwarderConfig, logger *zap.Logger) (*VictoriaMetricsForwarder, error) {
	if config.Endpoint == "" {
		return nil, fmt.Errorf("victoria-metrics endpoint is required")
	}
	...
	return &VictoriaMetricsForwarder{
		config: config,
		client: &http.Client{
...
func (f *VictoriaMetricsForwarder) Write(metrics []*Metric) error {
	...
	f.logger.Debug("Wrote to VictoriaMetrics",
...
func (f *VictoriaMetricsForwarder) convertToPrometheusFormat(metrics []*Metric) *prompb.WriteRequest {
...
// Close 关闭转发器
func (f *VictoriaMetricsForwarder) Close() error {
	f.client.CloseIdleConnections()
	return nil
}

// Name 获取转发器名称
func (f *VictoriaMetricsForwarder) Name() string {
	return f.config.Name
}

// Type 获取转发器类型
func (f *VictoriaMetricsForwarder) Type() ForwarderType {
	return ForwarderTypeVictoriaMetrics
}

// IsEnabled 是否启用
func (f *VictoriaMetricsForwarder) IsEnabled() bool {
	return f.config.Enabled
}

// GetStats 获取统计信息
func (f *VictoriaMetricsForwarder) GetStats() Stats {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.stats
}
...
// recordSuccess 记录成功
func (f *VictoriaMetricsForwarder) recordSuccess(bytes int64, latencyMs int64) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
// recordError 记录错误
func (f *VictoriaMetricsForwarder) recordError() {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.stats.FailedCount++
}
